...
### 🥗 AI Nutritionist (RAG-Powered)
Get a highly personalized daily fuel plan generated by AI, backed by verified nutrition data.
- **Retrieval Augmented Generation (RAG)**: Uses a custom vector database to fetch scientifically verified nutrition guidelines before generating plans.
- **Per-Meal Regeneration**: Don't like a specific meal? Click the **Regenerate (🔄)** button to get a new option without changing the entire plan.
- **Smart Variety**: Algorithms ensures diversity (e.g., rotating between Eggs, Parathas, and South Indian options for breakfast).
- **Custom Macros**: Protein, Carbs, and Fats calculated precisely for your body type.
- **Multi-Model Intelligence**: Automatically switches between **Gemini Flash 2.0** (Speed) and **Llama 3.3** (Reasoning) for optimal performance.
